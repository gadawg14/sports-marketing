...
?>
	...
	<div style="font: 9pt/9pt arial, tahoma; text-align: left; margin: 30px 0 0 0;">
		...
		$sql1 = mysql_query("SELECT count( * ) FROM `bbregistrations` WHERE campid = " . $_GET['id']);
		$totcampers = mysql_result($sql1,0);
		$sql2 = mysql_query("SELECT count( * ) FROM `bbregistrations`WHERE campid = " . $_GET['id'] . " AND paid <> ''");
...
while($row = mysql_fetch_array($sql)){ 
    ...
    echo '<td style="border-bottom: 1px solid black;"><a style="text-decoration: none;" href="ad_esmmain.php?i=ad_bbcampdetail.php&id=' . $row['id'] . '">' . $row['campername'] . '</td>' . "\n";
    ...
    echo '<td style="border-bottom: 1px solid black;">
    	<a href="ad_esmmain.php?i=ad_bbcampregdelete.php&id=', $row['id'] .'">Delete</a></td>', "\n";
        echo '</tr>';
} 
